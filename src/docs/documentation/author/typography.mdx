...
Using custom typography is very easy on a Superbook.
The only condition is that the typeface be web-compatible,
such as that from [Google Fonts API](https://developers.google.com/fonts/)
or other similar font hosting services. Checkout the section on
[best practices](/docs/practices) for better performance of your book.
